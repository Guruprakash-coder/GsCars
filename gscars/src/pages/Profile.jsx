...
const Profile = () => {
  ...
  const [passStep, setPassStep] = useState(1); // 1 = Send OTP, 2 = Verify
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {/* --- SECTION 2: PASSWORD FORM (CONDITIONAL) --- */}
        {showPassForm && (
          <div className="bg-slate-800 rounded-xl shadow-xl p-8 mb-8 text-white animate-fade-in-down">
            ...
            {passStep === 1 ? (
              <div className="text-center">
                <p className="text-gray-300 mb-6">
                  To change your password, we need to verify it's you. 
                  We will send a code to <strong>{user.email}</strong>.
                </p>
                <button 
                  onClick={sendOtp} 
                  disabled={loading}
              ...
              <form onSubmit={updatePassword} className="max-w-md mx-auto space-y-4">
                {passMsg && <div className="text-center text-green-400 font-bold mb-2">{passMsg}</div>}
            ...
            {passMsg && passStep === 1 && <div className="text-red-400 text-center mt-4">{passMsg}</div>}
          </div>
        )}
