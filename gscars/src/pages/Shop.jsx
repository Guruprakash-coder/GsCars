    ...
    {filteredProducts.map((product) => (
      ...
      >
        ...
        {product.originalPrice && product.originalPrice > product.price && (
          <div className="absolute top-2 right-2 bg-red-600 text-white text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 rounded shadow-lg z-20">
            {Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100)}% OFF
          </div>
        )}

        {/* Compatibility Badge (Smaller) */}
        <span className={`absolute top-2 left-2 px-2 py-0.5 text-[8px] md:text-[10px] font-bold uppercase tracking-wider text-white rounded-full z-20 
        ...
        <div className="p-3 md:p-5 flex flex-col flex-grow">
          ...
          <div className="mt-auto pt-2 border-t border-gray-100 flex items-center justify-between">
            <div className="flex flex-col">
              ...
              <span className="text-slate-900 font-extrabold text-base md:text-xl">₹{product.price}</span>
            </div>

            {/* WhatsApp Button (Smaller on mobile) */}
            <button 
              onClick={(e) => handleBuyClick(e, product.name, product.price)}
              className="bg-green-100 hover:bg-green-200 text-green-800 p-1.5 md:p-2 rounded-full transition-colors"
            >
